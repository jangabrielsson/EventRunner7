...
local function module(ER)
  ...
  local function createEventEngine()
    ...
    constraints['=='] = function(val) 
      return function(x) local a,b=coerce(x,val) return b~=nil and a == b end 
    end
  ...
  function setTimeout(fun,ms,errh)
    __assert_type(fun,LFUN) __assert_type(ms,"number")
...
MODULE[#MODULE+1] = { name = "Utils", sys=true, code = module }