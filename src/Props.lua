...
local function module(ER)
  ...
  local propertyTable = {
    ...
    profile = {
      trigger = {type='profile', property='activeProfile'},  -- TBD, should be a custom prop object
      get = function(pd,id) return api.get("/profiles").activeProfile end,
      set = function(pd,id,val,_) 
        if val then fibaro.profile("activateProfile",id) end 
        return val
      end
    },
    scene = {
...
MODULE[#MODULE+1] = { name = "Props", sys = true, code = module }