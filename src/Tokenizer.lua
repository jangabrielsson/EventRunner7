...
local function module(ER)
  ...
  local keywords = {
    ...
    [LFUN] = {type=LFUN,value=LFUN},
    ['end'] = {type='end',value='end'},
    ['if'] = {type='if',value='if'},
    [LTHEN] = {type=LTHEN,value=LTHEN},
    [LELSE] = {type=LELSE,value=LELSE},
    ['elseif'] = {type='elseif',value='elseif'},
    ['while'] = {type='while',value='while'},
    [LDO] = {type=LDO,value=LDO},
    ['loop'] = {type='loop',value='loop'},
    [LREPEAT] = {type=LREPEAT,value=LREPEAT},
  ...
  local tknsStrs = {
    ...
    {"0123456789", "%d%d%d%d/%d%d/%d%d/%d%d:%d%d:%d%d", function(s)
      ...
      local year,month,day,hour,min,sec = tonumber(y),tonumber(mo),tonumber(d),tonumber(h),tonumber(mi),tonumber(se)
      assert(year and month and day and hour and min and sec, "Invalid date literal: "..s)
      return {type='number', value=os.time({year=year,month=month,day=day,hour=hour,min=min,sec=sec})}
    end},
    {"0123456789", "%d%d%d%d/%d%d/%d%d/%d%d:%d%d", function(s)
      local y,mo,d,h,mi = s:match("(%d+)/(%d+)/(%d+)/(%d+):(%d+)")
      local year,month,day,hour,min = tonumber(y),tonumber(mo),tonumber(d),tonumber(h),tonumber(mi)
      assert(year and month and day and hour and min, "Invalid date literal: "..s)
      return {type='number', value=os.time({year=year,month=month,day=day,hour=hour,min=min,sec=0})}
    end},
    {"0123456789", "%d%d%d%d/%d%d/%d%d", function(s)
      local y,mo,d = s:match("(%d+)/(%d+)/(%d+)")
      local year,month,day = tonumber(y),tonumber(mo),tonumber(d)
      assert(year and month and day, "Invalid date literal: "..s)
      return {type='number', value=os.time({year=year,month=month,day=day,hour=0,min=0,sec=0})}
    end},
...
{"/", "/%d%d/%d%d/%d%d:%d%d:%d%d", function(s)
  ...
  local month,day,hour,min,sec = tonumber(mo),tonumber(d),tonumber(h),tonumber(mi),tonumber(se)
  assert(month and day and hour and min and sec, "Invalid date literal: "..s)
  return {type='number', value=os.time({year=os.date("*t").year,month=month,day=day,hour=hour,min=min,sec=sec})}
end},
{"/", "/%d%d/%d%d/%d%d:%d%d", function(s)
  local mo,d,h,mi = s:match("/(%d+)/(%d+)/(%d+):(%d+)")
  local month,day,hour,min = tonumber(mo),tonumber(d),tonumber(h),tonumber(mi)
  assert(month and day and hour and min, "Invalid date literal: "..s)
  return {type='number', value=os.time({year=os.date("*t").year,month=month,day=day,hour=hour,min=min,sec=0})}
end},
{"/", "/%d%d/%d%d", function(s)
  local mo,d = s:match("/(%d+)/(%d+)")
  local month,day = tonumber(mo),tonumber(d)
  assert(month and day, "Invalid date literal: "..s)
  return {type='number', value=os.time({year=os.date("*t").year,month=month,day=day,hour=0,min=0,sec=0})}
end},
{"+-*/%",".=",function(t) 
  local k = keywords[t]
  if not k then error("Bad token:"..t) end
...
MODULE[#MODULE+1] = { name = "Tokenizer", sys = true, code = module }