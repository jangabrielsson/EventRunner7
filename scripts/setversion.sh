...
for file_config in "${VERSION_FILES[@]}"; do
    ...
    if [ -f "$filepath" ]; then
        ...
        if grep -q "$pattern" "$filepath"; then
            # Use sed with capture group to preserve the matched pattern exactly
            # \1 captures and replays the matched pattern without the regex special chars
            sed -i.bak "s/\(${pattern}\) = \".*\"/\1 = \"$NEW_VERSION\"/" "$filepath"
